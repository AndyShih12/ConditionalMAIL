...
from conditionalMAIL.MTBCHanabiTest import MTBCHanabiTest
from envs.hanabi.hanabi_utils import getHanabiEnv, collect_expert_trajectories
...
def get_other_policies(mode, idx=None):
    if mode == "train":     rg = range(1,13)
    elif mode == "test":    rg = range(13,18)
    ...
    for load_dir in load_dirs:
        with open(load_dir + '/policy.pkl', 'rb') as f:
...
def ppo(ctxt, snapshot_dir, seed):
    ...
    env = getHanabiEnv(other_agents=None)
    ...
    with open(snapshot_dir + '/policy.pkl', 'wb') as f:
...
def mtbc(ctxt, snapshot_dir, seed):
    ...
    env = getHanabiEnv(other_agents=other_agents)
...
def mtbc_test(ctxt, snapshot_dir, load_dir, seed):
    ...
    for idx, other_agent in enumerate(other_agents):
        env = getHanabiEnv(other_agents=[other_agent])
        ...
        algo_fn = MTBCHanabiTest(env=env,
                learner=policy,
