...
from conditionalMAIL.MTBCParticleTest import MTBCParticleTest
from envs.particle.particle_utils import getParticleEnv, collect_expert_trajectories
...
def ppo(ctxt, snapshot_dir, seed):
    ...
    env = getParticleEnv(other_agents=None)

    algo_fn, policy = get_algo_fn(ARGS.algo, env, None, ARGS, continuous=True)

    runner = LocalRunner(ctxt)
    runner.setup(algo_fn, env)
    runner.train(n_epochs=TRAIN_EPOCHS*10, batch_size=TRAIN_BATCH//10)
...
def mtbc(ctxt, snapshot_dir, seed):
    ...
    env = getParticleEnv(other_agents=other_agents)

    trajs = [collect_expert_trajectories(policy=other_agent, env=env, fullbatch=10000) for other_agent in other_agents]

    algo_fn, policy = get_algo_fn(ARGS.algo, env, trajs, ARGS, continuous=True)
...
def mtbc_test(ctxt, snapshot_dir, load_dir, seed):
    ...
    for idx, other_agent in enumerate(other_agents):
        env = getParticleEnv(other_agents=[other_agent])
        ...
        algo_fn = MTBCParticleTest(env=env,
                learner=policy,
